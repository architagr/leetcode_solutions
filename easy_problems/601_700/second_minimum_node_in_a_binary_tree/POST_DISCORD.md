...
Every node in this tree has 0 or 2 children, and whenever it has 2, its value is the
smaller of the two children's values. That one rule guarantees the root always holds the
global minimum, and it means a subtree's minimum equals its own root's value too. Which
is the nice part: the moment we hit a node above the minimum, we record it as a
candidate **and stop descending into it**, since nothing underneath can be smaller.
...
too, but it's a leaf, so both calls hit the `nil` base case immediately. Nothing to
record down this branch.
...
branch does not recurse further. The subtree below it (5 and 7) can't beat 5, so it gets
pruned, this is the bit I actually like about this solution.

<img src="images/walkthrough-3.svg" alt="Step 3: candidate found, subtree pruned" width="360" />

**Step 4: return.** `ans = 5`, which is less than `math.MaxInt64`, so
`findSecondMinimumValue` returns `5`.

<img src="images/walkthrough-4.svg" alt="Step 4: final return value" width="360" />

**Why `[2,2,2]` gives `-1`:** every node equals `min` here, so `dfs` never leaves the
`else if` branch, it just walks to the leaves. `ans` stays at `math.MaxInt64` the whole
time, so the function falls through to `return -1`.

**Complexity:** O(n) time in the worst case, O(h) space for the recursion stack, where h
