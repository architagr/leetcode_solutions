...
func TestBinaryTreePaths(t *testing.T) {
	tests := []struct {
		...
		{name: "example 1", root: []any{1, 2, 3, nil, 5}, want: []string{"1->2->5", "1->3"}},
		{name: "example 2", root: []any{1}, want: []string{"1"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := binaryTreePaths(buildTree(tt.root)); !reflect.DeepEqual(got, tt.want) {
