...
func TestInvertTree(t *testing.T) {
	tests := []struct {
		...
		{name: "example 1", root: []any{4, 2, 7, 1, 3, 6, 9}, want: []any{4, 7, 2, 9, 6, 3, 1}},
		{name: "example 2", root: []any{2, 1, 3}, want: []any{2, 3, 1}},
		{name: "example 3", root: []any{}, want: []any{}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := treeVals(invertTree(buildTree(tt.root))); !reflect.DeepEqual(got, tt.want) {
