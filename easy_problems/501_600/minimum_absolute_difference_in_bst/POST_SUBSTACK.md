---
meta_title: "Smallest gap in a BST is always between neighbours"
...
"Minimum difference between any two nodes" reads like an all-pairs problem, and on an unsorted
structure it would be.

Two facts remove the quadratic, and neither is about trees specifically. One is what in-order
traversal of a BST gives you. The other is a property of sorted sequences worth carrying around
...
The predecessor to compare against is the previous node *in sorted order* — not the parent, not
whatever was last pushed onto a stack. Getting that wrong produces a solution that works on
balanced examples and quietly fails on a lean.
