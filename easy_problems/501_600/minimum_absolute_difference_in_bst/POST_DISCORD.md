...
order. In any sorted sequence, the minimum gap between two values always occurs
between neighbors — so instead of comparing every pair (O(n²)), just compare each
node to the *previous* node visited in the traversal and keep the smallest gap.
...
- Descend to leftmost node `1`. `prev` is nil → skip compare → `prev = 1`
