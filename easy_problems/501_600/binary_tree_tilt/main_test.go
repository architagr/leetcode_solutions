...
func TestFindTilt(t *testing.T) {
	tests := []struct {
		...
		{name: "example 1", root: []any{1, 2, 3}, want: 1},
		{name: "example 2", root: []any{4, 2, 9, 3, 5, nil, 7}, want: 15},
		{name: "example 3", root: []any{21, 7, 14, 1, 1, 2, 2, 3, 3}, want: 9},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := findTilt(buildTree(tt.root)); got != tt.want {
