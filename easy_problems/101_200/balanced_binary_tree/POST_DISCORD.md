...
subtree heights over and over, which costs O(n²) on a skewed tree. The fix: compute
height and balance in one bottom-up post-order pass, and short-circuit the moment any
...
base case `height=0, ok=true` for free, no special-casing needed.
...
Step 1: root's right subtree (leaf "2") resolves first, giving `(1, true)`.

![Step 1](images/walkthrough-1.svg)

Step 2: inside the left "2", its right child (leaf "3") resolves next, also `(1, true)`.
...
Step 5: the root combines `rightHeight=1` and `leftHeight=3`: `diff=2`, `|diff|>1`, so
`ok=false`. `isBalanced` returns `false`.

![Step 5](images/walkthrough-5.svg)

This runs in O(n) time, since each node is visited once, and O(h) space for the
recursion stack.
