...
func TestPreorderTraversal(t *testing.T) {
	tests := []struct {
		...
		{name: "example 1", root: []any{1, nil, 2, 3}, want: []int{1, 2, 3}},
		{name: "example 2", root: []any{1, 2, 3, 4, 5, nil, 8, nil, nil, 6, 7, 9}, want: []int{1, 2, 4, 5, 6, 7, 3, 8, 9}},
		{name: "example 3", root: []any{}, want: []int{}},
		{name: "example 4", root: []any{1}, want: []int{1}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := PreorderTraversal(buildTree(tt.root)); !reflect.DeepEqual(got, tt.want) {
