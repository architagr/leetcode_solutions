...
tags: [golang, binary-tree, recursion, strings, leetcode]
---
...
Serialising a tree to a string looks like a formatting exercise, and mostly it is. Then you
reach one clause in the rules that decides whether the whole thing works, and it is stated
so mildly it is easy to read straight past.

The rule is about when to keep an empty pair of parentheses. What it is really about is
whether the string you produce can be turned back into the tree it came from.
...
The lesson is about where a special case belongs. The rules here are asymmetric between
left and right, and the code that handles them cleanly is asymmetric in exactly the same
place. Write it symmetrically and you need an explicit "right child but no left child"
branch; write it the way the rules read and that branch never has to exist.
