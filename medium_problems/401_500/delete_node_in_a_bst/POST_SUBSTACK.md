...
tags: [golang, binary-search-tree, recursion, in-order, leetcode]
---
...
🔗 [LeetCode #450](https://leetcode.com/problems/delete-node-in-a-bst/) · Difficulty: Medium

Most of the BST problems in this batch have been about reading the structure. This is the
first one that changes it, and modifying a data structure with an invariant is a different
kind of exercise: every edit has to leave the invariant true.

The invariant here is the one we have been using all along, and it turns out to constrain the
answer so tightly that there are only ever two legal moves.
...
The move worth remembering is not the successor trick specifically. It is the habit of asking
what the invariant permits before asking how to code the change. Here the question "what may
fill this hole" has exactly two answers, and once you have them the implementation is forced.

Problems that mutate a structure usually work like that. Find the constraint, and the code
stops being a design decision.
