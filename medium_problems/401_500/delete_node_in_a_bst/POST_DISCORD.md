...
The problem splits itself: find the node, then delete it. Finding it is Day 45's walk — one comparison picks the only direction it could be in.
...
That follows straight from Day 52's property. If in-order has to stay sorted, the replacement must sit between everything in the left subtree and everything in the right — so it's the largest value on the left or the smallest on the right. Nothing else works.
