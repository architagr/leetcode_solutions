...
🔗 [LeetCode #235](https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-search-tree/) · Difficulty: Medium

Lowest common ancestor is a genuinely interesting problem on a general binary tree, and we
will do that version tomorrow. On a BST it is almost not a problem at all, and the gap
between the two is the point of doing them back to back.

Today's solution has no search in it. Not a faster search - none.
...
Reading this and tomorrow's side by side is the exercise. The same question, one guarantee
removed, and the solution goes from a walk down a single path to a traversal that has to
visit everything and report back.

That is what a BST's ordering actually buys, stated in code rather than in the abstract.
