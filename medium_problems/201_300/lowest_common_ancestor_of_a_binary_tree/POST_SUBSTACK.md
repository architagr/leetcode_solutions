---
meta_title: "LCA without the ordering: subtrees report upward"
...
Yesterday's problem had a guarantee attached to it, and today's is the same problem without
that guarantee. Reading the two solutions next to each other is more instructive than either
on its own.

The BST version had no search in it. This one has nothing but search. Same question, and
almost nothing survives the change.
...
It is worth being precise about what was lost. The BST version was O(h) in time because a
comparison eliminated a subtree; this one is O(n) because no comparison eliminates anything,
so every node has to be looked at before you can be sure.

Ordering is not a convenience in a BST. It is the thing that converts search into navigation,
and problems that look identical are a different difficulty on either side of it.
