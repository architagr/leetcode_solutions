...
This problem has a famous wrong answer, and the wrong answer is what you get by implementing
the problem statement exactly as written.

That is worth sitting with for a second, because it does not happen often. The definition is
correct and the literal translation of it is not, and the gap between those two is the whole
lesson.
...
There is a general habit here worth keeping. When a definition is awkward to check directly,
look for a property that is equivalent to it rather than implied by it. Implied gets you a
test that passes valid inputs; equivalent gets you a test that also rejects invalid ones.

In-order-is-sorted is equivalent, which is why one linear scan is a complete answer.
