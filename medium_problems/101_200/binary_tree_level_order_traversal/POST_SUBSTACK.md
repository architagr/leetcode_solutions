...
There is a solution to this problem that everyone writes, and it is not wrong. Queue, drain a
level, move to the next. If you have written BFS once you have written this.

Yesterday's problem was also per-level, and it was solved with a depth-first walk instead. The
same move works here, and this is the problem where it is worth asking why — because "level
order" sounds like the one thing a depth-first traversal cannot give you.
...
The thing to take from both days is that a traversal's arrival order and an answer's grouping
are separate concerns. When the accumulator is keyed on a property of the node rather than on
when it showed up, the traversal is free to arrive however it likes.

Where that stops being true is when the level boundary itself is part of the question. BFS knows
when a level ends; this recursion never does. Tomorrow's zigzag is exactly that kind of problem,
and it is worth noticing which shape it wants.
