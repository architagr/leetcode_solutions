...
🔗 [LeetCode #116](https://leetcode.com/problems/populating-next-right-pointers-in-each-node/) · Difficulty: Medium

There is a version of this problem that is entirely unremarkable, and most solutions are it:
walk each level left to right, hold on to the previous node, link forward. Nothing wrong with
it.

The version below changes one line and two awkward parts of the problem stop being parts of
the problem. It is a small thing, and it is the kind of small thing worth collecting.
...
Choosing a traversal direction to make an assignment natural is a cheap trick with a wide
reach. It shows up any time a problem asks about a neighbour: if the neighbour is ahead of
you, you need lookahead or a second pass, and if it is behind you, you already have it.

Reversing the walk is often the whole difference, and it costs a line.
