...
tags: [golang, binary-search-tree, data-structures, in-order, leetcode]
---
...
Most problems in this series have one answer worth writing. This one has two, and which is
better genuinely depends on facts the problem does not tell you.

That makes it a good interview problem and an awkward LeetCode problem: the accepted solution
and the thoughtful solution are not necessarily the same, and the difference is not about
being clever.
...
The question behind the question is about laziness. An iterator is an interface that lets a
caller stop early, and precomputing everything quietly removes that benefit while keeping the
interface. If callers usually consume the whole sequence, that costs nothing and buys
simplicity. If they usually stop early, it is the wrong shape.

Worth asking which situation you are in before deciding, rather than after.
