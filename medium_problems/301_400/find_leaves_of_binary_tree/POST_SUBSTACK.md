...
When a problem statement describes a procedure, there is a decision to make before writing any
code: implement the procedure, or work out what it produces.

Implementing it here is straightforward and slow. Working out what it produces takes one
observation, and the observation is more interesting than the problem, because it inverts the
measurement every other tree problem this month has used.
...
Depth and height are both "how far along a vertical path", and they are not interchangeable.
Depth is measured from the root down and is known on the way in; height is measured from the
leaves up and is only known on the way back out. Which one a problem needs decides whether the
answer is computed descending or ascending, and mixing them up produces code that looks right
and is off by exactly the shape of the tree.
