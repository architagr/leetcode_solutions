...
meta_description: "Finding the deepest level and summing it looks like two passes. A leaf deeper than any seen so far resets the sum instead of adding, so one suffices."
...
This is the last problem in the binary tree batch, and it is a fitting one to close on
because the solution is built entirely out of pieces from earlier days, arranged so that a
two-pass problem takes one pass.

The arrangement depends on a small idea that is easy to miss: an accumulator is allowed to be
wrong for a while, as long as it is discarded when better information arrives.
...
Fifty-one days of binary trees end here, and the shape that recurred most was this one:
carry something down, accumulate something up, and be precise about which. Depth goes down;
heights and sums come up. Level order needed no boundaries and zigzag did. The BST problems
were the same problems with a comparison that meant something.

Next batch moves on from trees. The habits carry over - most of what made these tractable was
deciding what each recursive call should return before writing a line of it.
