...
This is the first hard in the challenge, and it is a good one to arrive at now rather than
earlier, because the shape it needs was already built four days ago on an easy problem.

What makes it hard is not the traversal and not the arithmetic. It is that a single recursive
call has to produce one value and record a different one, and the two are close enough that
returning the wrong one still compiles, still runs, and is still wrong in a way the small
...
The pattern worth naming: when a recursion's answer can be centred anywhere, the recursive
call usually cannot return the answer. It returns whatever an ancestor needs in order to
extend, and the answer is accumulated on the side.

Diameter had it, this has it, and once you have seen it twice the third one stops looking
hard. The tell is a problem that says "does not need to pass through the root" - that clause
is telling you the return value and the answer are different things.
